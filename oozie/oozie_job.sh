...
oozie_host="http://172.18.0.2:11000/oozie/"
...
do
        ...
        for signal_file in $(ls -tr $process_dir/ | head -1); do
			...
			# if not valid move file to invalid signal directory and break loop
                        echo $signal
			oozie_job_id=$(oozie job -oozie $oozie_host -DSignalName=$signal -config job.properties -run) 
			...
			while $flag
			do
				job_status=$(oozie job -poll $oozie_job_id -interval 1 -oozie $oozie_host -verbose)
				case $job_status in
				RUNNING | SUBMITTED)
